...
import { Eye, Edit3, User, Mail, Power, Trash2, ShieldCheck } from "lucide-react";
import tempLogo from "../../../assets/images/temp_logo.png";

const DeptTable = ({ data, searchQuery, onEdit, onView, onToggleStatus, onDelete, selectedId, onSelect }) => {
  const filteredData = data.filter((item) => {
    const searchStr = searchQuery.toLowerCase();
    const name = item.dept_full_name || "";
    const shortName = item.dept_short_name || "";
    const id = item.dha_id || "";
        ...
        <table className="w-full text-left" style={{ borderCollapse: 'collapse' }}>
          <thead>
            <tr className="bg-slate-900 border-b border-slate-200">
              ...
              <th className="px-5 py-3.5 text-[10px] font-semibold uppercase tracking-widest text-slate-100 border-r border-slate-200/60">Organization</th>
              <th className="px-5 py-3.5 text-[10px] font-semibold uppercase tracking-widest text-slate-100 border-r border-slate-200/60">Department Head</th>
              <th className="px-5 py-3.5 text-[10px] font-semibold uppercase tracking-widest text-slate-100 border-r border-slate-200/60">Operation Manager</th>
          ...
          <tbody>
            {filteredData.length > 0 ? filteredData.map((item, index) => (
              <tr key={item.id} className={`group border-b border-slate-200 hover:bg-slate-50/50 transition-colors duration-200 ${selectedId === item.id ? 'bg-indigo-50/30' : ''}`}>
                <td className={`px-5 py-4 ${vBorder} text-center`}>
                  <input
                    type="checkbox"
                    className="w-4 h-4 rounded border-slate-400 text-indigo-600 focus:ring-indigo-500 cursor-pointer shadow-sm"
                ...
                <td className={`px-5 py-4 ${vBorder}`}>
                  <div className="flex justify-center">
                    <div className="w-10 h-10 rounded-xl bg-white border border-slate-200 flex items-center justify-center p-1.5 shadow-sm group-hover:border-indigo-200 group-hover:shadow-indigo-100/40 transition-all duration-400 overflow-hidden shrink-0">
                ...
                <td className={`px-5 py-4 ${vBorder}`}>
                  <div className="min-w-0">
                    <div className={`font-semibold text-[13px] tracking-tight group-hover:text-indigo-600 transition-colors truncate ${selectedId === item.id ? 'text-indigo-700' : 'text-slate-700'}`}>{item.dept_short_name}</div>
                    <div className="text-[9.5px] text-slate-700 font-semibold uppercase tracking-wider mt-0.5 truncate max-w-[150px]">{item.dept_full_name}</div>
                  </div>
                </td>
                <td className={`px-5 py-4 ${vBorder}`}>
                  <div className="min-w-0">
